...
- 任务：
  - [x] 补全 4 模式映射：`0 远程代理 / 1 本地直连 / 2 白名单模式 / 3 域名分流`（改 `MainViewModel` 两处，提取为共享纯函数避免重复）
  - [x] `DashboardScreen` 徽章颜色为"域名分流"补充配色（与 SshVpnService 日志文案一致）
  - [x] 确认 `switchDnsMode` 循环 `0→1→2→3` 在仪表盘点击后 4 个状态都能切换显示
...
- 任务：
  - [x] 梳理"默认服务器"唯一性约束：`setActive` 语义（一个且仅一个 active）应在保存/连接/断开三处一致
  - [x] 修复 `save()`：`setAsDefault=true` 时先 `deactivateAll()` 再 `setActive(id)`（或保证 `setActive` 的 CASE 原子性）；`setAsDefault=false` 时若该服务器当前是 active 需处理降级
  - [x] 仪表盘 `connectDefaultServer()` 与服务器列表 `connect(id)` 收敛到同一入口，active 状态单一数据源
  - [x] 校验"断开后"与"删除 active 服务器后"的 active 状态一致性（删除当前 active 后应有 fallback）
...
- 任务：
  - [x] 设计隧道管理新交互（独立于服务器编辑或嵌入编辑页）：类型选择 + 按 `configDescriptor` 动态渲染字段 + 配置 JSON 持久化回显
  - [x] 以 `TunnelPlugin.configDescriptor` 为唯一字段来源，移除硬编码 `TUNNEL_TYPES` 或改为由插件注册驱动
  - [x] `ServerEditViewModel`/表单支持 `tunnelConfigJson` 读写（新增字段时回显、保存时序列化）
  - [x] 展示插件能力标签（`TunnelCapability`）与默认值提示
