...
- **DNS**: `DnsInterceptor` REMOTE/WHITELIST mode fakes IPs (198.18.x.x / fd00::x) and maps `ipToDomain` for CONNECT; SYSTEM / DOMAIN_SPLIT (list miss) resolve via protected sockets. Non-A/AAAA queries (MX/TXT/PTR) in REMOTE mode fall back to a **protected-socket system-DNS lookup** (protect function is set unconditionally for all modes). `pendingResponses` is an **unlimited `Channel`** (not a polled queue) — `VpnController.dnsResponseDeliveryLoop` suspends on `receiveCatching()`. Fake-IP counters reset to the max remaining mapping after eviction, **clamped** to never fall below the allocation base (`198.18.0.0` / `fd00::2` — `fd00::1` is the VPN gateway).
