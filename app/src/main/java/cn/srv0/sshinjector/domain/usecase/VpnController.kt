...
class VpnController @Inject constructor(
    ...
    private val _logFlow = MutableSharedFlow<Pair<String, cn.srv0.sshinjector.ui.viewmodel.LogLevel>>(extraBufferCapacity = 10)
    ...
    fun addLog(message: String, level: cn.srv0.sshinjector.ui.viewmodel.LogLevel) {
    ...
    suspend fun connect(server: ServerConfig, password: String? = null): Result<Unit> {
        ...
        return try {
            ...
            if (transportMode == DnsInterceptor.DnsTransport.SYSTEM || transportMode == DnsInterceptor.DnsTransport.DOMAIN_SPLIT) {
                addLog(">>> [VpnController] 设置 DNS Interceptor 保护函数 ($transportMode)", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                dnsInterceptor.setProtectFunction { socket ->
                    addLog(">>> [VpnController] 保护函数被调用: socket=$socket", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
            ...
            if (transportMode == DnsInterceptor.DnsTransport.DOMAIN_SPLIT) {
                launch {
                    ...
                    addLog("域名列表刷新检查: ${if (needRefresh) "需要" else "无需"}", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                    if (needRefresh) {
            ...
            if (transportMode == DnsInterceptor.DnsTransport.SYSTEM) {
                ...
                if (systemDns.isNotEmpty()) {
                    addLog("SYSTEM 模式: 绕过 VPN 的 DNS 服务器: ${systemDns.joinToString(", ")}", cn.srv0.sshinjector.ui.viewmodel.LogLevel.INFO)
                } else {
    ...
    suspend fun updateDnsMode() {
        ...
        if (this.transportMode == DnsInterceptor.DnsTransport.SYSTEM || this.transportMode == DnsInterceptor.DnsTransport.DOMAIN_SPLIT) {
            addLog(">>> [VpnController] updateDnsMode: 设置 DNS Interceptor 保护函数 ($transportMode)", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
            dnsInterceptor.setProtectFunction { socket ->
                addLog(">>> [VpnController] 保护函数被调用: socket=$socket", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                val result = protectDatagramChannel?.invoke(socket) ?: false
                addLog(">>> [VpnController] 保护函数返回: $result", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
        ...
        addLog("DNS 模式已切换: $transportMode, 排除路由: ${excludedRoutes.size} 条", cn.srv0.sshinjector.ui.viewmodel.LogLevel.INFO)
    }
        ...
        try {
            ...
            if (transportMode == DnsInterceptor.DnsTransport.DOMAIN_SPLIT && dstIp != null && !isFakeIp(dstIp)) {
                ...
                if (direct) {
                    ...
                    addLog("域名分流直连: proto=$proto dst=${dstIp.hostAddress}:$dstPort", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                    writeToTun(data)
                    return
                }
            }
            ...
            when (workVersion) {
                ...
                6 -> {
                    ...
                    addLog(">>> [VpnController] 未知 IP 版本: $workVersion，丢弃", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                    // 未知版本，丢弃
                }
            }
        } catch (e: Exception) {
    ...
    private fun forwardDnsBypassPacket(buffer: ByteBuffer, dstIp: InetAddress, version: Int) {
        ...
        executor.submit {
            try {
                addLog(">>> [VpnController] forwardDnsBypassPacket: dstIp=$dstIp, version=$version, packetSize=${packetData.size}", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                val socket = java.net.DatagramSocket()
                val protected = protectDatagramChannel?.invoke(socket) ?: false
                addLog(">>> [VpnController] VpnService.protect()=$protected", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                if (!protected) {
                ...
                if (payloadLen <= 0) {
                    addLog(">>> [VpnController] payloadLen <= 0, 返回", cn.srv0.sshinjector.ui.viewmodel.LogLevel.WARNING)
                    return@submit
                }
                ...
                addLog(">>> [VpnController] 发送 DNS 查询到 $dnsServer:53, payload=${payload.size} bytes", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                socket.send(packet)
                ...
                addLog("<<< [VpnController] 收到 DNS 响应来自 ${responsePacket.address}:${responsePacket.port} (${responseData.size} bytes)", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                ...
                try {
                    ...
                    addLog(">>> [VpnController] 解析原始包: srcIp=$srcIp, srcPort=$srcPort", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                } catch (e: Exception) {
                    addLog(">>> [VpnController] 解析源 IP/端口失败: ${e.message}", cn.srv0.sshinjector.ui.viewmodel.LogLevel.ERROR)
                ...
                addLog(">>> [VpnController] 构造响应包完成: srcPort=53, dstPort=$srcPort, packetSize=${responsePkt.size}", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                writeToTun(responsePkt)
                addLog(">>> [VpnController] 已写回 TUN", cn.srv0.sshinjector.ui.viewmodel.LogLevel.DEBUG)
                socket.close()
                
            } catch (e: java.net.SocketTimeoutException) {
                addLog(">>> [VpnController] DNS 响应超时 (SocketTimeoutException)", cn.srv0.sshinjector.ui.viewmodel.LogLevel.WARNING)
            } catch (e: Exception) {
                addLog(">>> [VpnController] forwardDnsBypassPacket exception: ${e.message}", cn.srv0.sshinjector.ui.viewmodel.LogLevel.ERROR)
