...
class SshKeyManager
    ...
    constructor(
         ...
         */
        private fun parsePrivateKeyViaJsch(
            pem: String,
            passphrase: String?,
        ): Pair<String, String> {
            val jsch = JSch()
            ...
            try {
                val blob = keyPair.getPublicKeyBlob() ?: error("无法提取公钥")
        ...
        /** AES-GCM 加密写 .meta 文件 */
        private fun writeEncryptedMetaFile(
            file: File,
            content: String,
        ) {
            val wrappingKey = getOrCreateImportWrapperKey()
        ...
        private fun getOrCreateImportWrapperKey(): SecretKey {
            if (keyStore.containsAlias(importWrapperAlias)) {
                val entry =
                    keyStore.getEntry(importWrapperAlias, null) as? KeyStore.SecretKeyEntry
                        ?: error("无法读取导入密钥包装密钥")
        ...
        private fun loadImportedPem(fullAlias: String): ImportedPem? {
            ...
            return runCatching {
                ...
                require(payload.size <= MAX_IMPORTED_KEY_SIZE) { "imported key too large" }
                val passLen =
                ...
                var pos = 4
                require(passLen >= 0 && pos + passLen <= payload.size) { "invalid passphrase length" }
        ...
        fun getPublicKey(alias: String): String {
            ...
            val entry =
                keyStore.getEntry(fullAlias, null) as? KeyStore.PrivateKeyEntry
                    ?: error("密钥不存在: $fullAlias")
            val pubKey =
                entry.certificate?.publicKey
                    ?: error("无法提取公钥 (请重新生成密钥)")
        ...
        fun createJSchIdentity(
            ...
            try {
                ...
                val cert = entry.certificate ?: error("Certificate not found")
                val pubKey = cert.publicKey

                val publicKeyBytes =
                    when (pubKey) {
                        is java.security.interfaces.ECPublicKey ->
                            AndroidKeyStoreIdentity.buildEcdsaPublicKeyBlob(pubKey)
                        is java.security.interfaces.RSAPublicKey ->
                            AndroidKeyStoreIdentity.buildRsaPublicKeyBlob(pubKey)
                        is java.security.interfaces.EdECPublicKey ->
                            AndroidKeyStoreIdentity.buildEd25519PublicKeyBlob(pubKey)
                        else -> {
                            // 尝试通过算法名判断
        ...
        private fun logKeyAuthInfo(
            ignoredFullAlias: String,
        ...
        companion object {
            // PEM 缓存最大条目数，防止内存泄漏
            private const val MAX_PEM_CACHE_SIZE = 10

            // 导入私钥解密后 payload 上限 (PEM+口令)，防止解密出异常大对象
            private const val MAX_IMPORTED_KEY_SIZE = 4096
        }
    }
