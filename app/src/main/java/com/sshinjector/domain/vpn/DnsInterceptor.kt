...
class DnsInterceptor @Inject constructor() {
    ...
    // 用于 SYSTEM 模式：受保护的 DatagramChannel 工厂
    private var protectDatagramChannel: ((DatagramChannel) -> Boolean)? = null
    // 缓存的受保护通道，按 DNS 服务器 IP 分组
    private val protectedChannels = ConcurrentHashMap<String, DatagramChannel>()

    // 待发送的 DNS 响应映射 (queryId -> 待发送信息)
    private val pendingDnsResponses = ConcurrentHashMap<Int, DnsResponseInfo>()
    ...
    data class DnsResponse(
        val srcIp: InetAddress,    // DNS 服务器 IP (响应来源)
        val dstIp: InetAddress,    // VPN 客户端 IP (响应目标)
        val dstPort: Int,          // VPN 客户端源端口
        val data: ByteArray
    )

    data class DnsResponseInfo(
        val queryId: Int,
        val srcIp: InetAddress,
        val srcPort: Int,
    ...
    fun processDnsQuery(
        ...
        try {
            ...
            if (message.header.opcode != Opcode.QUERY) {
                android.util.Log.d(TAG, ">>> [DnsInterceptor] processDnsQuery: not QUERY opcode != QUERY, returning false")
                return false
            }
     ...
     */
    private fun sendDnsOverProtectedSocket(queryData: ByteArray, queryId: Int) {
        executor.submit {
            ...
            val dnsServer = systemDnsServers.firstOrNull()
     ...
     * 处理来自远程 DNS 的响应
     */
    fun onDnsResponse(queryId: Int, responseData: ByteArray) {
    ...
    private fun handleRemoteDnsFakery(
        ...
        if (existingIp != null) {
            ...
            fakeIp = "${(rawIp shr 24) and 0xFF}.${(rawIp shr 16) and 0xFF}.${(rawIp shr 8) and 0xFF}.${rawIp and 0xFF}"
            fakeInetAddress = InetAddress.getByName(fakeIp)
        ...
        val answer: Record = if (qtype == org.xbill.DNS.Type.A) {
    ...
    private fun sendErrorResponse(pending: DnsPendingQuery, rcode: Int) {
